...
setwd("~/Research/covid19/results/2020-04-01_excess_ili_by_state/")
...
ili <- read_csv("../../data/ILINet/week12/ILINet.csv", skip=1, na="X")
ili_flu_clinical <- read_csv("../../data/ILINet/week12/WHO_NREVSS_Clinical_Labs.csv", skip=1, na="X")
ili_flu_combined <- read_csv("../../data/ILINet/week12/WHO_NREVSS_Combined_prior_to_2015_16.csv", skip=1, na="X")
...
ggsave("non_influenza_ili_eda.pdf", height=7, width=7, units="in")
...
# upswing_bool <- function(x, run_limit=0){
#  if (length(x) <= 1+run_limit) return(rep(TRUE, length(x)))
#  start <- 1
#  run_total <- 0
#  for (i in 2:length(x)){
#     if (x[i] < x[i-1]){
...
#       run_total <- 0
#     }
#     if (run_total > run_limit) { start <-  i+1}
#  }
#  if (start >= length(x)) return(rep(FALSE, length(x)))
#  c(rep(FALSE, (start-1)), rep(TRUE, length(x)-(start-1)))
# }
...
upswing_bool <- function(x){
  ...
  # stopcrit <- FALSE
  # while(!stopcrit){
  #   madx <- ma(dx[start:length(x)], 2)
  #   if (any(madx < 0)) {
  #     start <- start + min(which(madx < 0))
  #   } else {
  #     stopcrit <- TRUE  
  #   }
  # }
...
Y_ncov_summary_tidy  %>% 
  ...
  write_csv("ncov_signal_extraction.csv")


# # Calculate percentage of observed ILI due to covid
# ili_flu_select %>% 
#   mu


# Output scaled to total population number of excess ILI cases in the state
# tmp <- Y_ncov_summary_tidy  %>% 
#   select(REGION, week, p2.5:p97.5, mean, p.positive) 
# foo <- rep(0, nrow(tmp))
# for (i in 1:nrow(tmp)){
#   foo[i] <- scale_factor[unlist(tmp[i,"REGION"]), test_idxs[unlist(tmp[i,"week"])]]
# }
# tmp %>% 
#   mutate(scale_factor = foo) %>% 
#   mutate(p2.5 = p2.5*scale_factor, 
#          p25 = p25*scale_factor, 
#          p50 = p50*scale_factor, 
#          p75 = p75*scale_factor, 
#          p97.5 = p97.5*scale_factor, 
...
Y_ncov_full_scaled_summary %>% 
  ...
  ungroup() %>% 
  mutate(week = CDC_date[test_idxs][week]) %>% 
  write_csv("number_excess_ili_cases.csv")
rm(foo, tmp)  
...
Y_predict_summary %>% 
  ...
  geom_point(data=Y_ncov_summary_tidy, color="blue", size=.5) +
  ...
  ylim(c(0, NA))
ggsave("ncov_signal_extraction.pdf", height=9, width=7, units="in")
...
Y_predict_summary %>% 
  ...
  theme(axis.text.x=element_text(angle=90, hjust=1), 
        axis.title.x = element_blank())
ggsave("ncov_signal_extraction_washington.pdf", height=5, width=7, units="in")
...
# foo <- rep(0, nrow(tmp))
# for (i in 1:nrow(tmp)){
#   foo[i] <- rpois(1,scale_factor[unlist(tmp[i,"REGION"]), test_idxs[unlist(tmp[i,"week"])]])
# }
# tmp <- tmp %>% 
#   mutate(scale_factor = foo) %>% 
#   mutate(p2.5 = p2.5*scale_factor, 
...
# foo2 <- rep(0, nrow(tmp2))
# for (i in 1:nrow(tmp2)){
#   foo2[i] <- rpois(1, scale_factor[unlist(tmp2[i,"REGION"]), test_idxs[unlist(tmp2[i,"week"])]])
# }
# tmp2 <- tmp2 %>% 
#   mutate(scale_factor = foo2) %>% 
#   mutate(p2.5 = p2.5*scale_factor, 
#          p25 = p25*scale_factor, 
#          p50 = p50*scale_factor, 
#          p75 = p75*scale_factor, 
...
Y_ncov_full_scaled_summary %>% 
  ...
  ylab("Non-Influenza ILI in Excess of Seasonal Norms")
ggsave("correlation_covid_excess_ili_by_week.pdf", height=10, width=10)
...
ggsave("excess_ili_vs_case_counts.pdf", height=7, width=7, units="in")
...
ggsave("excess_ili.pdf", height=15, width=20, units="in")
...
sims <- read_csv("weekly_I_across_replicates_4d_lag.csv") %>% 
  select(-X1) %>% 
...
total_new_cases <- sum(filter(us_new_confirmed_tidy, CDC_week==13)[["NewCasesWeek"]])
tmp %>% 
...
for (i in 1:ncol(alex_sims)){
  sim_probs[i] <- d1(alex_sims[1,i])*d2(alex_sims[2,i])
  #sim_probs[i] <- d2(alex_sims[2,i])
}
any(!is.na(sim_probs))
...
data.frame("replicate" = 1:length(sim_probs), "probability" =non_zero_prob[1,]) %>% 
  write_csv("distribution_over_replicates.csv")
...
ili_sims %>% 
  ...
  mutate(date = d[date]) %>% 
  write_csv("posterior_samples_us_weekly_I.csv")
